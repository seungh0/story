...
class FeedMappingModifier(
    ...
    suspend fun modify(
        ...
        val configuration = feedMappingConfigurationRepository.findById(request.toConfigurationKey())
            ?: throw FeedMappingNotFoundException("워크스페이스(${request.workspaceId})의 리소스(${request.resourceId})의 컴포넌트(${request.componentId})의 이벤트(${request.eventAction})에 대해서 구독(${request.subscriptionComponentId})와 피드 연동 설정이 없습니다")
